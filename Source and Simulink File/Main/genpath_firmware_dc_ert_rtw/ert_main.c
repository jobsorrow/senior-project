...
void rt_OneStep(void)
{
   ...
   */
  genpath_firmware_dc_SetEventsForThisBaseStep(eventFlags);
  enableTimer0Interrupt();
  genpath_firmware_dc_step0();
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(genpath_firmware_dc_M, 0);
  genpath_firmware_dc_initialize();
  initializeOverrunService();
  globalInterruptDisable();
  configureTimer0(modelBaseRate, systemClock);
  runModel = rtmGetErrorStatus(genpath_firmware_dc_M) == (NULL);
  enableTimer0Interrupt();
  enable_interrupts();
  globalInterruptEnable();
  while (runModel) {
    stopRequested = !(rtmGetErrorStatus(genpath_firmware_dc_M) == (NULL));
  }

  /* Terminate model */
  genpath_firmware_dc_terminate();
  globalInterruptDisable();
  return 0;
}
